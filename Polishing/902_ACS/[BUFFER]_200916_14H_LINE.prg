...
MFLAGS(AXIS).11=0				! Dynamic brake mode disable
!*****************************************************************************
...
TILL FAULT(AXIS).#LL | IN(0).0 = 1, (TimeOut * 60)       ! TimeOut 60sec 
IF ^FAULT(AXIS).#LL & IN(0).0 = 0
...
JOG (AXIS), +
TILL ^FAULT(AXIS).#LL & IN(0).0 = 0, (TimeOut * 60)       ! TimeOut 60sec 
IF FAULT(AXIS).#LL & IN(0).0 = 1
  DISP" X-Axis move to search limits fault "
  GOTO Time_Out
END
...
HomeFlag(AXIS)=1
DISP "X-Axis Home Done = %d, Axis number=%d" ,HomeFlag(AXIS), AXIS
!*****************************************************************************
...
! 15 *************************************************************************
Time_Out:
DISP "X-Home fault time_out, Axis number=",AXIS
HALT (AXIS)
...
FDEF(AXIS).#RL=1  
FDEF(AXIS).#CPE=1
DISP" X-Axis safety all enable, Axis Nomber = ", AXIS
RET
!*****************************************************************************
...
TILL MST(AXIS).#ENABLED 
JOG (AXIS), -                                           ! Move to left limit  at low speed  
TILL FAULT(AXIS).#LL | IN(0).1 = 1, (TimeOut * 60)       ! TimeOut 60sec 
IF ^FAULT(AXIS).#LL &  IN(0).1 = 0
...
JOG (AXIS), +
TILL ^FAULT(AXIS).#LL & IN(0).1 = 0, (TimeOut * 60)       ! TimeOut 60sec 
IF FAULT(AXIS).#LL & IN(0).1 = 1
  DISP" Axis move to search limits fault "
  GOTO Time_Out
END
...
! 9 **************************************************************************
! Move to pin up position
PTP/E AXIS, Home_Offset(Polishing_Y)
TILL ^MST(AXIS).#MOVE
WAIT 1000
SET FPOS(AXIS) = FPOS(AXIS) - FPOS(AXIS)
!*****************************************************************************
...
DISABLE(AXIS)
WAIT 2000
TILL ^MST(AXIS).#ENABLED & FVEL(AXIS) < 3
...
IF MFLAGS(AXIS).#HALL = 0
	!IF MFLAGS(AXIS).#BRUSHOK=0
		...
		TILL MST(AXIS).#ENABLED
		WAIT 500
		COMMUT (AXIS), Commut_current, 2000		! Encoder commutation command
		TILL MFLAGS(AXIS).9, (TimeOut*10)		
		IF ^MFLAGS(AXIS).9
  			DISP"Axis commutation fault"
  			GOTO Time_Out
...
TILL MST(AXIS).#ENABLED 
JOG (AXIS), -                                            ! Move to left limit  at low speed  
TILL FAULT(AXIS).#LL | IN(1).0 = 1, (TimeOut * 60)       ! TimeOut 60sec 
IF ^FAULT(AXIS).#LL & IN(1).0 = 0
  DISP" Axis move to search limits fault "
  GOTO Time_Out
END
...
JOG (AXIS), +
TILL ^FAULT(AXIS).#LL & IN(1).0 = 0, (TimeOut * 60)       ! TimeOut 60sec 
IF FAULT(AXIS).#LL & IN(1).0 = 1
  DISP" Axis move to search limits fault "
  GOTO Time_Out
END
...
HomeFlag(AXIS) = -1		! Reset homeflag
DATA_ACS_TO_EQ(AXIS) = 0
...
Home_Dir(AXIS) = 0      ! 0: left(-) limit, 1: right(+) limit
...
!*****************************************************************************
! Disable motor
DISABLE (AXIS); TILL ^MST(AXIS).#ENABLED
...
! Homing method
IF     Home_Dir(AXIS) = 0; COEWRITE/1 (ECAT_NODE_NO, 0x6098, 0, 11) ! 1 => Move to LEFT  limit then index
ELSEIF Home_Dir(AXIS) = 1; COEWRITE/1 (ECAT_NODE_NO, 0x6098, 0, 12) ! 2 => Move to RIGHT limit then index
ELSE   GOTO L_ERROR
END
...
VelMoveToSwitch  = VelMoveToSwitch / EFAC(AXIS)
VelMoveToZero    = VelMoveToZero   / EFAC(AXIS)
AccHoming        = AccHoming       / EFAC(AXIS)
Home_Offset(Cleaning_T)		 = Home_Offset(Cleaning_T)	   / EFAC(AXIS) * (-1)
...
SET FPOS(AXIS) = ActualPosition(ECAT_NODE_NO) * EFAC(AXIS)
! Homing start
...
L_ERROR:
	...
	HALT AXIS;    TILL ^MST(AXIS).#MOVE
	DISABLE AXIS; TILL ^MST(AXIS).#ENABLED 
	HomeFlag(AXIS) = -1
	DATA_ACS_TO_EQ(AXIS) = 1
STOP

!*****************************************************************************
! Auto routine for external stop command
!*****************************************************************************
ON ^PST(AXIS).#RUN & HomeFlag(AXIS) <> 1
	! Axis Number
...
VelMoveToZero                = VelMoveToZero   / EFAC(AXIS)
AccHoming                    = AccHoming       / EFAC(AXIS)
Home_Offset(Polishing_Theta) = Home_Offset(Polishing_Theta)	/ EFAC(AXIS) * (-1)
...
VelMoveToZero    = VelMoveToZero   / EFAC(AXIS)
AccHoming        = AccHoming       / EFAC(AXIS)
Home_Offset(Polishing_Tilt)		 = Home_Offset(Polishing_Tilt)	   / EFAC(AXIS) * (-1)
...
VelMoveToZero           = VelMoveToZero   / EFAC(AXIS)
AccHoming               = AccHoming       / EFAC(AXIS)
Home_Offset(Storage_Y)	= Home_Offset(Storage_Y)	   / EFAC(AXIS) * (-1)
...
VelMoveToZero           = VelMoveToZero   / EFAC(AXIS)
AccHoming               = AccHoming       / EFAC(AXIS)
Home_Offset(Cleaning_Y)	= Home_Offset(Cleaning_Y) / EFAC(AXIS) * (-1)
...
if Force_Flag = 1
    LOOP COUNT
		VEL(2)=20; ACC(2)=1000; DEC(2)=1000; JERK(2)=10000
	    PTP/E 2, SOFT_START_POS									!Start Position
		WAIT 500
		!PTP/f 2, SOFT_START_POS, 5							!Move to Softlanding Position
		PTP/v 2, (SOFT_START_POS + Detect_POS), Detect_VEL	!Detect Force Index
	!	WAIT Detect_Time
...
ECIN(560, Diag(0))
ECIN(561, SICK_DATA(0)) !EC_PLCIN(0)
ECIN(571, SICK_DATA(1)) !EC_PLCIN(1)
ECOUT(446, EC_PLCOUT(0))

!Beckhoff Digital Input Card EL1889
ECIN(       629 ,EC_IN( 0))
ECIN(       630 ,EC_IN( 1))
ECIN(       631 ,EC_IN( 2))
ECIN(       632 ,EC_IN( 3))
...
!Beckhoff Digital Output Card EL2889
ECOUT(       456 ,EC_OUT( 0))
ECOUT(       457 ,EC_OUT( 1))
ECOUT(       458 ,EC_OUT( 2))
ECOUT(       459 ,EC_OUT( 3))
...
!SMC JXCE1 Drive PDO Data, Node 33
...
!SMC JXCE1 Drive PDO Data, Node 34
